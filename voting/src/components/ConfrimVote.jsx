    ...
    const closeCandidateModal = () => {
      ...
      const currentVoter = useSelector((state) => state?.vote?.currentVoter);
        ...
        const fetchCandidate = async () => {
            try {
                const res = await axios.get(`${import.meta.env.VITE_API_URL}/candidates/${selectedVoteCandidate}`, {withCredentials: true,
        ...
        const confirmVote = async() => {
            try {
                const res = await axios.patch(`${import.meta.env.VITE_API_URL}/candidates/${selectedVoteCandidate}`, {selectedElection},{withCredentials: true,
                headers: { Authorization: `Bearer ${token}`},
                });
